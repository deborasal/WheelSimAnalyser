...
clc;
disp('Step 5 - Data Visualisation Started');
...
for i = 1:numel(numericalCols)
    ...
    % Subplot 3: Bar Chart
    subplot(2, 2, 3);
    meanValues = varfun(@mean, plotData, 'InputVariables', 'Value', ...
        'GroupingVariables', {'Experiment', 'Period'});
    barData = reshape(meanValues.mean_Value, [], length(unique(plotData.Period)));
    bar(barData, 'grouped');
    set(gca, 'XTickLabel', experimentNames);
    title(['Bar Chart - ', upper(metricStrFormatted)], 'FontWeight', 'bold');
    ylabel('Mean Value');
    xlabel('Experiment');
    legend(unique(plotData.Period), 'Location', 'Best');

    % Subplot 4: Violin Plot
    subplot(2, 2, 4);
    if exist('violin', 'file') == 2
        violin({T_experiment{1}.(metric), T_experiment{2}.(metric)}, ...
            'xlabel', experimentNames, 'facecolor', [0.5 0.5 1; 1 0.5 0.5], 'edgecolor', 'none');
        title(['Violin Plot - ', upper(metricStrFormatted)], 'FontWeight', 'bold');
        ylabel('Value');
    else
        warning('Violin function not available.');
    end
